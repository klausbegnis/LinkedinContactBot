        ...
        for i in range(len(links)):
            ...
            try:
                ...
                if self.driver.find_element_by_xpath('//header[@class = "pv-contact-info__header t-16 t-black t-bold"]'):
                    ...
                    sectionExperiencia = self.driver.find_elements_by_xpath('//section[@id = "experience-section"]/ul//li')
                    secao1 = sectionExperiencia[0]

                    """ tag_p = secao1.find_elements_by_tag_name('p')
                    for elements in tag_p:
                        print(elements.text) """
                    sState = 'var'
                    try:
                        tag_s = secao1.find_elements_by_tag_name('span')
                        for s in range(len(tag_s)):
                            if tag_s[s].text == "Nome da empresa":
                                trabalho = tag_s[s+1].text
                                sState = 0
                                s = len(tag_s) + 10
                    except:
                        pass
                    if sState != 0:
                        try:
                            tag_p = secao1.find_elements_by_tag_name('p')
                            for p in range(len(tag_p)):
                                if tag_p[p].text == "Nome da empresa":
                                    trabalho = tag_p[p+1].text
                                    p = len(tag_p) + 10
                        except:
                            pass
